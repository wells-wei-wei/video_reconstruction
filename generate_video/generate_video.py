...
if __name__ == "__main__":
    ...
    test_opt = TestOptions().parse()
    test_opt.gpu_ids=0
    if not os.path.exists("../demo/results/frames"):
        os.system("mkdir ../demo/results/frames")
    test_opt.output_dir="../demo/results/frames"
    test_opt.src_path="../demo/transfer_data/000000.jpg"
    test_opt.tgt_path="../demo/src/frames/"
    test_opt.bg_ks = 13
    test_opt.ft_ks = 3
    ...
    with open(pkl_path, 'rb') as f:
        human_body_info = pickle.load(f)

    print('\n\t\t\tImitating `{}`'.format(test_opt.tgt_path))
    tgt_paths = scan_tgt_paths(test_opt.tgt_path, itv=1)
    imitator.generate_video(human_body_info, tgt_smpls=None, cam_strategy='smooth',output_dir=test_opt.output_dir, visualizer=None, verbose=True)
